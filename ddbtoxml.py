...
def genXML(character):
    ...
    senses = []
    for modifier in (
            modifiers["race"] + modifiers["class"] + modifiers["background"] + modifiers["item"] + modifiers["feat"] +
            modifiers["condition"]):
    ...
    background = ""
    if "background" in character and character["background"] is not None and character["background"][
...
def findURLS(fp):
    ...
    characters = []
    regex = re.compile(
        "<a[^>]*href=\"(/profile/.*/[0-9]+)\"[^>]*class=\"ddb-campaigns-character-card-header-upper-details-link\"[^>]*>")
    for line in fp:
        m = regex.search(line)
        if m:
